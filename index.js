...
client.on('message', async function (topic, message) {   //khi có message đến
    ...
    try {
        ...
        const data = await temperatureModel.find().sort({ 'createdAt': 'DESC' }).limit(5).select('temperature -_id'); //lấy 5 giá trị mới nhất từ data base
...
io.on('connection', async socket => {
    try {
        const data = await temperatureModel.find();
